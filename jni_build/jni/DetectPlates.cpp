...
PossiblePlate extractPlate(cv::Mat &imgOriginal, std::vector<PossibleChar> &vectorOfMatchingChars) {
	...
	LOGI("before getRectSubPix");

	tlx = possiblePlate.rrLocationOfPlateInScene.center.x - possiblePlate.rrLocationOfPlateInScene.size.width/2;
	tly = possiblePlate.rrLocationOfPlateInScene.center.y - possiblePlate.rrLocationOfPlateInScene.size.height/2;
	if(tlx <= 0)
		tlx = 10;
	if(tly <= 0)
		tly = 10;
	if((tlx + possiblePlate.rrLocationOfPlateInScene.size.width) >= imgRotated.cols)
	{
		ROIW = imgRotated.cols - tlx - 10;
	}
	else
		ROIW = possiblePlate.rrLocationOfPlateInScene.size.width;

	if((tly + possiblePlate.rrLocationOfPlateInScene.size.height) >= imgRotated.rows)
	{
		ROIH = imgRotated.rows - tly - 10;
	}
	else
		ROIH = possiblePlate.rrLocationOfPlateInScene.size.height;

	imgCropped = imgRotated(cv::Rect(tlx, tly, ROIW, ROIH));
